...
Page({
  ...
  getQuestion: function(){
    var that = this
    if(that.data.isTodayAnswer == true)
    {
      db.collection("dailyQuestionBank").where({  	
        ...
        success(res) {
          ...
          that.setData({
            stem:res.data.stem,
            questionPicBase:res.data.image,
            optionOne:res.data.option1,
            optionTwo:res.data.option2,
            optionThree:res.data.option3,
            tureAnswer:res.data.tureAnswer,
